...
## TO DO: How should we change this so it only if for discrete variables? We want to use a cmap for continous variables and a palette for discrete 
```{r}
...
for (col in meta_cat) {
    ...
    if (length(unique_values) > 0) {
        col_palette <- setNames(kelly.colors((length(unique_values))), unique_values)
...
## Read in images
```{r}
images <- loadImages("../../data/img/") # PUT IN CONFIG
```
